...
Once the status shows `SUCCESSFUL`, you can then retrieve the presigned URL using the [headless import URL endpoint](/api-reference/headless/get-presigned-download-url-for-headless-import-data):
...
- A [step hook](/reference/hooks#instanceabortimport) awaited `instance.abortImport({ code, message })`

When a step hook aborts the import, `import_errors` on the [retrieve headless import](/api-reference/headless/retrieve-headless-import) response contains the same `{ code, message }` you passed in. Display those fields in your UI the same way the widget does.

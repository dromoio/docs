---
title: "Welcome to Dromo"
description: "The intuitive spreadsheet importer that seamlessly integrates with your product"
icon: "rocket"
---

<img
  className="block dark:hidden"
  src="/images/hero-light.png"
  alt="Hero Light"
/>
<img
  className="hidden dark:block"
  src="/images/hero-dark.png"
  alt="Hero Dark"
/>

Dromo is an easy and intuitive spreadsheet importer that integrates seamlessly with your product. Dromo helps your users match columns, correct errors, perform complex validations, transform values, and upload perfectly formatted data.

<Card title="🚀 Try Our Live Demo" icon="play" href="https://demo.dromo.io">
  Experience Dromo with realistic data and see how easy data importing can be
</Card>

## Quick Start

Get up and running with Dromo in minutes. Choose the integration that works best for your stack:

<CardGroup cols={2}>
  <Card title="JavaScript SDK" icon="js" href="/getting-started/javascript">
    Universal JavaScript implementation for any web application
  </Card>
  <Card title="React Component" icon="react" href="/getting-started/react">
    Pre-built React component with TypeScript support
  </Card>
  <Card title="Angular Component" icon="angular" href="/getting-started/angular">
    Native Angular implementation with full type safety
  </Card>
  <Card title="Schema Builder" icon="wrench" href="/guides/schema-builder">
    No-code schema creation in the Dromo Dashboard
  </Card>
</CardGroup>

## See Dromo in Action

<iframe
  src="https://drive.google.com/file/d/1dv39s4HZ88U9uI7PVARHet4T9z6qjf_1/preview"
  width="640"
  height="480"
  allow="autoplay"
></iframe>

## How Dromo Works

<Steps>
  <Step title="Define Your Schema">
    Specify the fields you want to import using our [Schema Builder](/guides/schema-builder) or define them programmatically with our SDKs.
  </Step>
  <Step title="Add Validations">
    Configure [validation rules](/fields) and [transformation hooks](/hooks) to ensure data quality and provide real-time feedback.
  </Step>
  <Step title="Handle Results">
    Receive clean, validated JSON data that's ready to use in your application or database.
  </Step>
  <Step title="Customize Experience">
    Brand the importer to match your application with custom styling and user experience settings.
  </Step>
</Steps>

## What You Need to Get Started

<AccordionGroup>
  <Accordion title="Free License Key">
    Get started immediately with a free license key by [registering for an account](https://dashboard.dromo.io/auth/registration). No payment information required.
  </Accordion>
  <Accordion title="JavaScript Environment">
    Dromo works with any JavaScript application or HTML page where you want users to import data.
  </Accordion>
  <Accordion title="Supported File Types">
    Import CSV, TSV, XLS, XLSX, XML, or JSON files directly into your application.
  </Accordion>
</AccordionGroup>

## Key Features

<CardGroup cols={3}>
  <Card title="Smart Column Matching" icon="arrows-turn-to-dots">
    Intelligent algorithms help users map their columns to your schema
  </Card>
  <Card title="Real-time Validation" icon="shield-check">
    Catch and fix errors before data enters your system
  </Card>
  <Card title="Data Transformation" icon="wand-magic-sparkles">
    Clean and format data automatically with custom hooks
  </Card>
  <Card title="White-label Ready" icon="palette">
    Fully customizable to match your brand and user experience
  </Card>
  <Card title="Enterprise Security" icon="lock">
    SOC 2 compliant with enterprise-grade security features
  </Card>
  <Card title="Developer Friendly" icon="code">
    Comprehensive APIs and SDKs for complete integration control
  </Card>
</CardGroup>

## Next Steps

<CardGroup cols={2}>
  <Card title="Interactive Sandbox" icon="laptop-code" href="https://codesandbox.io/s/ux-demo-react-yez5e2">
    Explore a fully customized Dromo implementation
  </Card>
  <Card title="API Documentation" icon="book-open" href="/api-reference">
    Dive into our comprehensive API reference
  </Card>
  <Card title="Join Community" icon="users" href="https://discord.gg/dromo">
    Connect with other developers building with Dromo
  </Card>
  <Card title="Get Help" icon="life-ring" href="mailto:contact@dromo.io">
    Our team is here to help you succeed
  </Card>
</CardGroup>

<Note>
  **Questions?** Chat with us anytime using the chat bubble, join our [Discord community](https://discord.gg/dromo), or email us at [contact@dromo.io](mailto:contact@dromo.io).
</Note>

...
description: "Configure manyToOne fields to map a variable number of uploaded spreadsheet columns into a single Dromo schema field."
---

This guide demonstrates how to use field setting to map multiple columns to a single field.

<Info>
  For more detail about the `manyToOne` setting, see the [field object reference](/reference/fields/fields#field-object-reference).
</Info>
...
[Step hooks](/reference/hooks#step-hooks) `REVIEW_STEP` and `REVIEW_STEP_POST_HOOKS` can be used to inspect column mapping at run time to see which columns were mapped to `manyToOne` fields.

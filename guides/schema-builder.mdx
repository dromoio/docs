...
* **Webhooks**: Configure [webhooks](/reference/webhooks) to receive notifications when imports complete.
* **Allow Custom Fields**: Enable the [allowCustomFields setting](/reference/settings/settings#allowcustomfields) to let users add fields not in your schema.
* **Invalid Data Behavior**: Configure how to handle invalid data using the [invalidDataBehavior setting](/reference/settings/settings#invaliddatabehavior).
* **Results**: Configure how to handle [results](/reference/results) when imports complete.

...
description: "Customize the Dromo Uploader's colors, typography, buttons, steps, modals, and other visual elements with style overrides."
---
...
- **`disabledBackgroundColor`** - Fill color when the button is disabled (default: `#e5e7eb`)
- **`disabledTextColor`** - Text color when the button is disabled (default: `#6b7280`)
- **`padding`** - Button padding (default: `8px 12px`)
...
- **`errorBorderColor`** - Border color when the control is in an error state
...
- **`activeBorderColor`** - Border color while a file is being dragged over the dropzone
- **`acceptBorderColor`** - Border color when the dragged file is accepted
- **`rejectBorderColor`** - Border color when the dragged file is rejected
...
const settings = {
  styleOverrides: {
    dropzone: {
      ...
      acceptBorderColor: "#10b981",
      rejectBorderColor: "#ef4444"
    }
  }
}
```
...
The Error Navigator is available when [`reviewStep.enableNavigatingErrors`](/reference/settings/settings#enablenavigatingerrors) is enabled. Its colors and icons can be customized independently from the data table.

- **`errorColor`** - Color of error indicators, messages, and the error count
- **`warningColor`** - Color of warning indicators and messages
- **`accentColor`** - Color of primary actions and the active column filter
- **`errorIconUrl`** - URL of a custom error icon, or `null` for the default
- **`warningIconUrl`** - URL of a custom warning icon, or `null` for the default

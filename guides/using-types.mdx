...
By default, if you don't specify a type for a field, it uses the [`string` type](/fields/field-types#string%E2%80%8B). For example, below, we create a field for collecting email addresses:
...
However, if we update the "Email address" field to use the [`email` type](/fields/field-types#email%E2%80%8B), that field gains the advantage of Dromo's built-in email address validation:
...
For example, we want a field that tracks prices in USD. We can add a "Price" field using the ["usd" preset](/fields/field-types#number%E2%80%8B):
...
![Validation error for inputting invalid currency](/images/guides/assets/images/use_wrong_currency.png)

<Note>
  For numbers (and a few other field types) the display format is not what the field outputs. If you want to change how a field is output, the `outputFormat` configuration option enables you to do so. For example, if you wish to display *and* output a field into USD, you can use the ["usd" preset](/fields/number-formatting#usd) alongside an `outputFormat` option.

...
<Update label="September 2026" description="Upcoming" tags={["Customization", "Bulk Fix", "Hooks", "Headless API"]}>

## Abort an import from a step hook

Step hooks can now stop an import immediately with a structured error.

- **`await instance.abortImport({ code, message })`**: Call from any async step hook (or `beforeFinish`) with a string `code` and `message`. The widget shows both. In headless, the import is marked `FAILED` and `import_errors` contains the same `{ code, message }`.

See [`instance.abortImport`](/reference/hooks#instanceabortimport) in the Hooks reference.

## Optional Bulk Fix Step

- **`skipBulkFixStep`**: Set to `true` to always skip the Bulk Fix step, regardless of whether select, multi-select, or date values would normally cause it to appear.

See [`skipBulkFixStep`](/reference/settings/settings#skipbulkfixstep) in the Settings reference for configuration details.

</Update>

<Update label="August 2026" description="v3.13.0" tags={["Performance", "Data Processing"]}>

## Batch Row Hooks for Large Imports

Large imports can now be transformed in bounded chunks during initialization, avoiding the need to pass the entire dataset to one bulk callback.

- **`registerBatchRowHook()`**: Register a JavaScript SDK callback that receives one slice of rows and its one-based batch number
- **`batchRowHooks`**: Supply the same initialization-only callbacks through the React prop or Angular input
- **`batchRowHookBatchSize`**: Set the maximum rows per callback invocation with a positive integer; defaults to `2000`

See the [Batch row hooks reference](/reference/hooks#batch-row-hooks) for callback signatures and examples.
...
Sessions now support a short-lived signed **`hydrationId`** token as a more secure alternative to passing the raw `sessionId` UUID in API URLs. The `sessionId`-based approach remains supported — `hydrationId` is an opt-in improvement that limits the impact of a leaked credential by scoping access to a short-lived token.

- **`hydrationId`**: A signed token returned on session create; pass it to the widget instead of the raw `sessionId`
- **`expiresAt`**: Token expiration timestamp returned by the Sessions API; configure the lifetime with the `expires_in` request field
- **`sessionId`**: Unchanged long-lived identifier — store it server-side to re-issue a fresh `hydrationId` whenever needed

See the [Session Saving & Restoration Guide](/guides/saving-progress) for the full security model and endpoint reference.
...
- **Authentication**: Authenticate SFTP API requests with a backend license key in the `X-Dromo-License-Key` header
...
- **Light and Dark Examples**: Build a light or dark appearance from individual style override keys using the documented examples

...
- **`instance.abortImport({ code, message })`**: Call from any step hook (or `beforeFinish`) with a string `code` and `message`. The widget shows both. In headless, the import is marked `FAILED` and `import_errors` contains the same `{ code, message }`.

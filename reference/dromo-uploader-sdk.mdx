...
<ResponseField name="registerBulkRowHook(callback: BulkRowHookFn) => void">
  Used to register a [bulk row hook](/reference/hooks#bulk-row-hooks) callback.
</ResponseField>

<ResponseField name="registerBatchRowHook(callback: BatchRowHookFn) => void">
  Registers an initialization-only [batch row hook](/reference/hooks#batch-row-hooks). The callback receives one bounded slice of rows and its one-based batch number. Configure the maximum slice size with [`batchRowHookBatchSize`](/reference/settings/settings#batchrowhookbatchsize).
</ResponseField>

<ResponseField name="registerStepHook(stepHookType: &quot;UPLOAD_STEP&quot; | &quot;REVIEW_STEP&quot; | &quot;REVIEW_STEP_POST_HOOKS&quot; | &quot;REVIEW_STEP_PRE_SUBMIT&quot;, callback: StepHookFn) => void">
  Used to register a [step hook](/reference/hooks#column-hooks) callback of the given type.
</ResponseField>

<ResponseField name="registerFileParser(parser: { extensions: string[], parseFile: ParseFileFn }) => void">
  Registers a [custom file parser](/reference/custom-file-parsers) for one or more file extensions.
</ResponseField>

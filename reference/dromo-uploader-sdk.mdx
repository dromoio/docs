...
<ResponseField name="registerBatchRowHook(callback: BatchRowHookFn) => void">
  Registers an initialization-only [batch row hook](/reference/hooks#batch-row-hooks). The callback receives one bounded slice of rows and its one-based batch number. Configure the maximum slice size with [`batchRowHookBatchSize`](/reference/settings/settings#param-batch-row-hook-batch-size).
</ResponseField>

...
<ResponseField name="savePartialSession" type="boolean" default="false">
  ...
  **Note:** Completed imports automatically clear their saved session data. Use with [`sessionId`](#sessionid) to restore a saved session.
...
<ResponseField name="sessionId" type="string" default="null">
  ...
  **Restoration Behavior:**
  - Fetches the saved session from the backend using the provided session ID
  ...
  **Note:** Use with [`savePartialSession`](#savepartialsession) enabled to create sessions that can be restored.

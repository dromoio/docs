...
<ResponseField name="title" type="string" default='Add ${importIdentifier}'>
  ...
  ![Importer title customized with the title setting](/images/assets/images/title.png)
...
<ResponseField name="invalidDataBehavior" type='"BLOCK_SUBMIT" | "REMOVE_INVALID_ROWS" | "INCLUDE_INVALID_ROWS"' default='REMOVE_INVALID_ROWS'>
  ...
  | :----------------------------------------------------------------------------------------: | :-----------------------------------------------------------------------------------------: | :-------------------------------------------------------------------------------: |
  | ![Remove invalid rows confirmation](/images/assets/images/invalidData_remove_invalid.png) | ![Include invalid rows confirmation](/images/assets/images/invalidData_include_invalid.png) | ![Blocked submit when invalid data exists](/images/assets/images/invalidData_block.png) |
...
<ResponseField name="batchRowHookBatchSize" type="number" default="2000">
  Sets the maximum number of rows passed to each [batch row hook](/reference/hooks#batch-row-hooks) invocation. The value must be a positive integer.

  This setting has no effect unless a batch row hook is registered. Batch row hooks run only once during import initialization and receive a one-based batch number as their second argument.
</ResponseField>
...
<ResponseField name="styleOverrides" type="object" default="{}">
  Customizes the importer's colors, typography, controls, and other visual elements. See the complete [style overrides reference](/reference/settings/style-overrides).
</ResponseField>

<ResponseField name="textOverrides" type="object" default="{}">
  Overrides supported error and interface text.

  <div className="ml-8">
    <ResponseField name="errors" type="Record<string, string>" default="{}">
      Maps supported error keys to replacement messages.
    </ResponseField>

    <ResponseField name="ui" type="Record<'COLUMN_MATCH_SUBTITLE' | 'BULK_FIX_SUBTITLE', string>" default="{}">
      Overrides the subtitle shown on the Column Matching or Bulk Fix step.
    </ResponseField>
  </div>
  ...
  settings: {
    textOverrides: {
      ui: {
        COLUMN_MATCH_SUBTITLE: "Match each spreadsheet column to a destination field",
        BULK_FIX_SUBTITLE: "Review and standardize unmatched values",
      },
    },
  }
  ```
...
<ResponseField name="skipBulkFixStep" type="boolean" default="false">
  If `true`, Dromo always skips the Bulk Fix step, even when select, multi-select, or date fields would otherwise require bulk fixing. This setting takes precedence over the automatic conditions that normally show the step.

  ```javascript
  skipBulkFixStep: true
  ```
</ResponseField>
...
<ResponseField name="xmlParsingOptions" type="object">
  Configures client-side parsing for XML uploads.

  <div className="ml-8">
    <ResponseField name="forceArrayTags" type="string[]">
      XML tag names that should always be treated as arrays, including when only one matching element exists.
    </ResponseField>

    <ResponseField name="headerAlias" type="Record<string, string>">
      Maps XML tag names to the column headers Dromo should display.
    </ResponseField>

    <ResponseField name="enableAutoArrayHeuristics" type="boolean" default="true">
      Automatically recognizes plural parent and singular child tags, such as `<sales><sale>`, as arrays.
    </ResponseField>

    <ResponseField name="assumeSingleRootChildIsArray" type="boolean" default="true">
      Treats a single object nested beneath a single root tag as a one-row dataset.
    </ResponseField>
  </div>
  ...
  settings: {
    xmlParsingOptions: {
      forceArrayTags: ["contact"],
      headerAlias: { email_address: "Email" },
      enableAutoArrayHeuristics: true,
      assumeSingleRootChildIsArray: true,
    },
  }
  ```
</ResponseField>
...
<ResponseField name="passThroughUnmappedColumns" type="boolean" default="false">
  With this setting enabled, Dromo will include all columns not mapped or added as custom fields in the result payload.
  ...
  Additionally, the `metadata` object passed to your `resultsCallback` will include a `rawHeaders` array containing the original column headers in order, allowing you to map column indexes to their actual header names.
  ...
  // metadata.rawHeaders
  ["Name", "Last Name", "Email", "Phone", "Country", "Zip", "Birthday", "Notes"]
  ```

  <Note>
    For a complete guide on working with unmapped columns including how to use `rawHeaders` to convert column indexes to header names, see the [Accessing Unmapped Columns guide](/guides/accessing-unmapped-columns).
  </Note>
</ResponseField>
...
The `settings.uploadStep` object contains settings for the Upload Step experience. It defaults to `{}`.
...
<ResponseField name="templateDownloadOverrideURL" type="string" default="null">
  If set, the template download button will download from this URL instead of generating a CSV template from your schema fields.

  This allows you to provide a custom template file that may include additional formatting, instructions, or example data beyond what can be captured in the basic CSV template.

  The file will be downloaded with the same filename that would be used for the generated template.

  ```javascript
  uploadStep: {
    templateDownloadOverrideURL: "https://your-domain.com/custom-template.xlsx"
  }
  ```
</ResponseField>
...
<ResponseField name="transformDataPrompts" type="string[] | null" default="null">
  An array of custom prompt strings displayed as clickable suggestions in the AI "Transform Data" modal. When set to a non-empty array, these strings replace the built-in localized prompts shown to the user. `null` or an empty array falls back to the default prompts.

  Requires [`enableUserTransformations`](#enableusertransformations) to be `true`.

  ```javascript
  settings: {
    reviewStep: {
      enableUserTransformations: true,
      transformDataPrompts: [
        "Remove all rows where the email domain is not @company.com",
        "Normalize all phone numbers to E.164 format",
        "Fill blank 'country' cells with 'United States'",
      ],
    }
  }
  ```
...
* Adjusted phone numbers
  ...
  <Expandable title="an example">
    ![Automatically fixed cells highlighted in the review table](/images/assets/images/highlightAutoFixes.png)
  </Expandable>
...
<ResponseField name="enableNavigatingErrors" type="boolean" default="false">
  When enabled, adds an error navigation panel to the Review Step that lets users move through validation errors one at a time and fix them inline.

  **Row headers**: Any row with a validation error displays a red circular info icon next to its row number. Clicking the icon opens the navigator panel anchored to that row's first error.

  **Navigator panel**: Slides in from the right of the table and lists all validation errors as cards — each showing the row number, column name, and error message.

  - **Column filter**: A dropdown filters the list to a single column.
  - **Prev/Next navigation**: Buttons and the `Alt+↑` / `Alt+↓` keyboard shortcuts step through errors in order, scrolling the table to each cell.
  - **Jump to cell**: Clicking any error card scrolls directly to that cell.
  - **Inline actions** (shown on the active error card):
    - **Clear** — removes the invalid cell value. For multi-select fields, only the invalid tokens are removed.
    - **Replace** — replaces the cell value with a new one. For select fields a valid-options dropdown is shown; for multi-select fields the chosen value is appended to the existing valid tokens; for all other fields a free-text input is shown.
  - **Closing the panel** (X button) resets the panel state and column filter.

  ![Error navigator panel showing validation errors with inline Clear and Replace actions](/images/assets/images/navigator.png)
...
<ResponseField name="savePartialSession" type="boolean" default="false">
  Requires [backendSyncMode](#backendsyncmode) = "FULL_DATA"

  When enabled, Dromo will automatically save user progress throughout the import flow, allowing users to resume their session if they navigate away or close the browser.

  **Auto-Save Behavior:**
  - Progress is automatically saved to the backend with intelligent debouncing (~700ms after changes)
  - Captures the complete state at each stage: upload, header selection, column matching, and data review
  - Includes column mappings, transformations, row data, and the uploaded file
  - Sessions are stored server-side and linked to a unique session identifier

  **Use Cases:**
  - Long-running imports where users may need to step away
  - Complex data mappings that take time to configure
  - Preventing data loss from accidental browser closures
  - Multi-session workflows where users return to complete imports later

  **Note:** Completed imports automatically clear their saved session data. Use [`hydrationId`](#hydrationid) for new integrations, or [`sessionId`](#sessionid) for the legacy flow, to restore a saved session. See the [Session Saving & Restoration Guide](/guides/saving-progress) for the full security model and API reference.
...
<ResponseField name="sessionId" type="string" default="null">
  Requires [backendSyncMode](#backendsyncmode) = "FULL_DATA"

  Restores a previously saved import using the legacy session identifier. Sessions remain available for seven days after the most recent save.

  **Restoration Behavior:**
  - Fetches saved state from the backend using the raw session identifier and the widget's frontend license key
  - Validates that the session matches the current `importIdentifier`
  - Downloads and restores the uploaded file data
  - Rehydrates the UI to the exact stage where the user left off:
    - **Sheet Selection** → Returns to upload step
    - **Header Selection** → Restores headers and jumps to header selection
    - **Column Matching** → Restores mappings and jumps to column matching
    - **Data Review** → Restores full data grid and jumps to review step
  - Preserves all user work including mappings, transformations, and data edits

  For new integrations, prefer [`hydrationId`](#hydrationid), which avoids exposing the long-lived session identifier to the browser. Store `sessionId` on your backend so it can create a fresh hydration token when the user returns.

  **Use Cases:**
  - Resume workflows where users receive a session link to continue an import
  - Multi-step approval processes where different users complete different stages
  - Scheduled or delayed import completion

  **Note:** Use with [`savePartialSession`](#savepartialsession) enabled to create sessions that can be restored. See the [Session Saving & Restoration Guide](/guides/saving-progress) for the full token lifecycle and API reference.
...
<ResponseField name="hydrationId" type="string" default="null">
  Requires [backendSyncMode](#backendsyncmode) = "FULL_DATA"

  A short-lived signed access token used by the widget to create, read, update, and delete saved session state. The token identifies one session, so the widget does not need the raw `sessionId` when `hydrationId` is provided.

  Create a session from your backend with `POST /api/widget/session/v2/auth/`. When a user returns, mint a fresh token with `POST /api/widget/session/v2/{sessionId}/auth/`. Delete a session with `DELETE /api/widget/session/v2/{sessionId}/`.

  Hydration tokens default to a six-hour lifetime. Set `expires_in` in the create or resume API request to choose a different lifetime; this is an API parameter, not a widget setting. The underlying session expires seven days after its most recent save.

  <Warning>
    Call the session create, resume, and delete endpoints only from your backend with a backend license key. Pass the returned `hydrationId` to the widget, but never expose the backend key.
  </Warning>
  ...
  See the [Session Saving & Restoration Guide](/guides/saving-progress) for the complete token lifecycle and API examples.
</ResponseField>

...
<ResponseField name="allowMultiUploads" type="boolean" default="false">
  Allows a user to select multiple CSV or Excel files in the Upload Step and combine them into one import.

  Dromo previews the first file in the Header Selection Step. When the user selects a header row, Dromo applies that same row number to every selected file, creates the union of all headers, and aligns each file's rows by header name. Columns missing from a file are filled with empty values. Column matching, validation, review, hooks, and result delivery then run once against the combined dataset.

  ```javascript
  const settings = {
    importIdentifier: "contacts",
    allowMultiUploads: true,
  };
  ```

  Named headers must be unique within each file. If an Excel workbook contains multiple sheets, its selected sheet must have the same name as the sheet selected in the first workbook, or you must configure [`uploadStep.sheetOverride`](#param-sheet-override).
</ResponseField>
...
<ResponseField name="maxRecords" type="number" default="null">
  Specify the maximum number of records an import can contain. This value does not include file headers. When [`allowMultiUploads`](#param-allow-multi-uploads) is enabled, the limit applies to the combined rows from all selected files.
...
<ResponseField name="sessionId" type="string" default="null">
  ...
  When provided, Dromo will restore a previously saved import session, allowing users to resume exactly where they left off.

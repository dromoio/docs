...
<ResponseField name="savePartialSession" type="boolean" default="false">
  ...
  **Note:** Completed imports automatically clear their saved session data. Use with [`sessionId`](#sessionid) to restore a saved session. See the [Session Saving & Restoration Guide](/guides/saving-progress) for the full security model and API reference.
...
<ResponseField name="sessionId" type="string" default="null">
  ...
  **Restoration Behavior:**
  - Fetches saved state from the backend using a `hydrationId` token derived from the session ID
  ...
  **Security model:** Session state is accessed via a short-lived signed `hydrationId` token (5-hour rolling TTL, 24-hour absolute cap) rather than the raw session UUID. When `sessionId` is set, the widget exchanges it for a fresh `hydrationId` via `POST /api/widget/session/<id>/authenticate/` before fetching state. Store the raw `sessionId` in your backend or durable client storage; keep the `hydrationId` in memory only.

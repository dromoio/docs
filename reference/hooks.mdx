---
title: "Hooks"
description: "Reference for column, row, bulk row, and step hooks, including callback inputs and outputs, row-cell transformations, dynamic fields with addField, validation messages, and lifecycle timing."
---
...
<Tip>
  Looking for simpler validation options? Check out [field validators](/reference/fields/field-validators) for built-in validation rules, or see [validating with external data](/guides/validate-external-data) for a row hook example.
</Tip>

The diagram below shows the general flow for Dromo. Column hooks run first and only once after the "column matching" stage. Batch row hooks run next during initialization, followed by standard and bulk row hooks. Standard and bulk row hooks also run after the user updates data; batch row hooks do not.

![Hook execution order throughout the Dromo import flow](/images/assets/images/hooks_flow_chart.png)
...
Row hooks run validation on imported records and return updated data and/or user messages. They come in three variants: standard, batch, and bulk.

<Note>
  To register row hooks, use `registerRowHook()`, `registerBatchRowHook()`, or `registerBulkRowHook()` in the [JavaScript SDK](/reference/dromo-uploader-sdk). React and Angular integrations provide the corresponding `rowHooks`, `batchRowHooks`, and `bulkRowHooks` props or inputs.
</Note>

Standard and bulk row hooks run once in `"init"` mode after the column matching step and before the user begins the review step. They run again in `"update"` mode after the user changes data. Batch row hooks run only during initialization and receive bounded slices of the dataset.
...
### Batch row hooks[​](#batch-row-hooks "Direct link to Batch row hooks")

Batch row hooks process large imports in bounded slices instead of passing the full dataset to one callback. They run only once during initialization, after column hooks and before standard and bulk row hooks.

The callback receives two arguments:

<ResponseField name="records" type="object[]">
  A slice of row hook record objects. The maximum number of records is controlled by [`batchRowHookBatchSize`](/reference/settings/settings#batchrowhookbatchsize), which defaults to `2000`.
</ResponseField>

<ResponseField name="batchNumber" type="number">
  The one-based number of the current batch. The first callback receives `1`, the second receives `2`, and so on.
</ResponseField>

Batch row hooks do not receive a mode argument because they never run in `"update"` mode. Return an array of row hook output objects with each record's original `index`. You may return only the fields that should change; omitted fields and rows remain unchanged. Async callbacks are supported.

<Accordion title="Batch row hook example">
  <CodeGroup>
  ```javascript JavaScript
  const settings = {
    importIdentifier: "Large Contact Import",
    batchRowHookBatchSize: 5000,
  };

  const dromo = new DromoUploader("FRONTEND_API_KEY", fields, settings, user);

  dromo.registerBatchRowHook(async (records, batchNumber) => {
    const enrichedEmails = await enrichEmailBatch(records, batchNumber);

    return records.map((record) => ({
      index: record.index,
      row: {
        email: { value: enrichedEmails[record.index] },
      },
    }));
  });
  ```
  ...
  ```typescript Angular
  import { IBatchRowHook } from "dromo-uploader-angular";

  batchRowHooks: IBatchRowHook[] = [
    async (records, batchNumber) => {
      console.log(`Processing batch ${batchNumber}`);
      return records;
    },
    ...
    instance.updateInfoMessages([
      {
        ...
        fieldKey: "month",
        messages: [
          { level: "info", message: "Sales exceed previous month" },
        ],
      },
    ]);
...
### `instance.abortImport`[​](#instanceabortimport "Direct link to instanceabortimport")

Stops the import with a structured error. Await the returned promise so the hook does not continue before the abort is acknowledged.

The widget shows the `code` and `message`. Closing the error dialog cancels the uploader.

In a [headless import](/getting-started/headless#import-failed), calling `abortImport` marks the import as `FAILED`. The same `{ code, message }` object is returned in `import_errors`, so you can display the identical error in your own UI.

<ResponseField name="error" type="{ code: string, message: string }" required>
  <ResponseField name="code" type="string" required>
    A short error code you define, for example `"BAD_FORMAT"`.
  </ResponseField>
  <ResponseField name="message" type="string" required>
    Message shown to the user in the widget, and stored on the failed headless import.
  </ResponseField>
</ResponseField>

<Accordion title="abortImport example">
  <CodeGroup>
  ```javascript JavaScript
  dromo.registerStepHook("UPLOAD_STEP", async function (instance, data) {
    if (!data.filename?.endsWith(".csv")) {
      await instance.abortImport({
        code: "BAD_FORMAT",
        message: "The file does not match the expected format.",
      });
    }
  });
  ```

...
<ResponseField name="batchRowHooks" type="BatchRowHook[]" href="/reference/hooks#batch-row-hooks">
  An array of initialization-only [batch row hook](/reference/hooks#batch-row-hooks) functions. Each function receives a bounded slice of rows and its one-based batch number.
</ResponseField>

<ResponseField name="rowDeleteHooks" type="RowDeleteHook[]" href="/reference/hooks#row-delete-hooks">
  An array of [row delete hook](/reference/hooks#row-delete-hooks) functions called when users remove rows.
</ResponseField>
...
<ResponseField name="stepHooks" type="object[]" href="/reference/hooks#step-hooks">
  ...
  <div className="ml-8">
    <ResponseField name="type" type="'UPLOAD_STEP' | 'REVIEW_STEP' | 'REVIEW_STEP_POST_HOOKS' | 'REVIEW_STEP_PRE_SUBMIT'" required>
...
<ResponseField name="customSteps" type="CustomStepConfig[]" href="/reference/custom-steps">
  An array of [custom step](/reference/custom-steps) configurations inserted into the import flow.
</ResponseField>

<ResponseField name="fileParsers" type="{ extensions: string[], parseFile: ParseFileFn }[]" href="/reference/custom-file-parsers">
  An array of [custom file parser](/reference/custom-file-parsers) configurations.
</ResponseField>
...
<ResponseField name="onUserEvent" type="(event: IUserEvent) => void" href="/reference/on-user-event">
  Called when the user interacts with a tracked control in the importer. See the [onUserEvent reference](/reference/on-user-event) for event payloads.
</ResponseField>

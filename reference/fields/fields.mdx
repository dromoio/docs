...
<ResponseField name="validators" type="Validator[]">
  An array of validator objects to validate this field. See [field validators](/reference/fields/field-validators).
</ResponseField>

<ResponseField name="selectOptions" type="{ label: string, value: string, alternateMatches?: string[] }[]">
  For select fields only, an array of possible picklist values. See the [select field type](/reference/fields/field-types#select).
</ResponseField>
...
<ResponseField name="requireMapping" type="boolean" default="false">
  ...
  Note: cells in this column aren't required to have any values; that can be validated using [the `required` validator](/reference/fields/field-validators#required).
</ResponseField>

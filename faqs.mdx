---
title: "FAQs"
description: "Find answers to common questions about Dromo's spreadsheet importer, security, supported files, integrations, billing, and product capabilities."
---
...
Dromo is a Delaware C corporation based in San Francisco. We support customers on six continents and over 30 languages (see the full list [here](/reference/settings/settings#locale)).
...
No. A user has the option to skip uploading a file and enter their data manually (copying and pasting is supported) as long as the [`manualInputDisabled`](/reference/settings/settings#manualinputonly) flag is not set.
...
Yes, but a user cannot upload the file directly to Dromo. You must parse the file and provide the contents to Dromo via the [`initialData`](/reference/settings/settings#initialdata) setting.
...
Yes, you can achieve this with the [`autoMapHeaders`](/reference/settings/settings#automapheaders) setting. When enabled, the importer will remember mappings of prior imports that used the same `importIdentifier`.
...
You may either handle the the [`onResults`](/reference/results) callback directly in your application, or you may use a [webhook](/reference/webhooks). In that case, Dromo will send a POST request to any address you specify in the [`webhookUrl`](/reference/settings/settings#webhookurl) setting whenever an import is completed.
...
Yes, you can customize the look-and-feel of the importer using the [`styleOverrides`](/reference/settings/settings#styleoverrides) setting.
...
Yes. In Dromo Pro, error messages for built-in [validators](/reference/fields/field-validators) can be customized by providing an `errorMessage` in the JavaScript SDK. You may also write [custom messages](/reference/hooks#working-with-info-messages), including error messages, to any cell using row or column hooks.

You may customize the help text at each step (i.e., Upload Step, Matching Step, and Review Step) by providing the appropriate [`helpText`](/reference/settings/settings#helptext) setting.

## Security[​](#security "Direct link to Security")

### What is Private Mode?[​](#what-is-private-mode "Direct link to What is Private Mode?")

In Private Mode, the data file uploaded by the end user is processed entirely in the context of their web browser, and the results are shared back to your application directly. To enable Private Mode, set [`backendSyncMode`](/reference/settings/settings#backendsyncmode) to `DISABLED`. The Dromo Pro SDK enables Private Mode by default.
...
If you set [`backendSyncMode`](/reference/settings/settings#backendsyncmode) to `MAPPINGS_ONLY`, then information about column headers will be shared with Dromo (this is required to enable certain advanced features like AI-assisted column mapping and remembering mappings from prior imports), but the rest of the file is not shared with Dromo.
...
### How long does Dromo retain imported data?[​](#how-long-does-dromo-retain-imported-data "Direct link to How long does Dromo retain imported data?")

Dromo retains imported data according to your organization's configured retention period. You can manage this period in the Organization section of the [Dromo Dashboard](https://dashboard.dromo.io). Retention is indefinite by default, and custom retention periods are configurable.

### Does Dromo support single sign-on (SSO)?[​](#does-dromo-support-single-sign-on-sso "Direct link to Does Dromo support single sign-on (SSO)?")

Yes. Dromo supports Google SSO today, and support for additional identity providers is in progress.
